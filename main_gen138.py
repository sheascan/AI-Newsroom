...
import numpy as np
import requests
import email # NEW: For parsing EML files
import email.policy # NEW: For parsing EML files
...
# --- VERSION TRACKER ---
VERSION_ID = "Gen 138 (Embedding Batch Fix)"
...
class ContentClusterer:
    ...
    def run_clustering(self, articles, embeddings):
        ...
        # --- BUILD THE RAW CLUSTERS DICTIONARY FIRST ---
        raw_clusters = {}
...
# --- HELPER: EMBEDDINGS (UPDATED: BATCHING) ---
def get_embeddings_batch(texts):
    all_embeddings = []
    chunk_size = 100 # Gemini API limit
    try:
        # Loop through chunks
        for i in range(0, len(texts), chunk_size):
            ...
            # Extend list with results from this chunk
...
# --- STAGE 1 & 2: SCRAPING (UPDATED FOR EML & QP STITCHING) ---
                    ...
                    for part in msg.walk():
                        ...
                        if ctype in ["text/plain", "text/html"]:
                            try: 
                                ...
                                # --- GEN 137: STITCH QUOTED-PRINTABLE SPLITS ---
                                payload = re.sub(r'=\r?\n', '', payload)
                                content += payload + "\n"
                            except: pass
                else:
...
async def main():
    ...
    # NOTE: Since inputs were archived in the last failed run, 
    # we might need to look in the archive folder if INPUT_DIR is empty.
    # But for standard safety, we stick to INPUT_DIR.
