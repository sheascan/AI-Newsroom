...
# --- VERSION TRACKER ---
VERSION_ID = "Gen 32 (The Script Vault & Audio Fallback)"
...
SCRIPT_DIR = os.path.join(CACHE_DIR, "scripts") # <--- NEW: Vault for Scripts
...
async def produce_audio(topic, script, filename):
    ...
    for i, line in enumerate(script):
        ...
        # --- AUDIO ENGINE with FALLBACK (Gen 32) ---
        segment = None
        
        # 1. Try Microsoft Edge (High Quality)
        ...
        if segment is None and HAS_GTTS:
            print("      ⚠️ EdgeTTS Failed. Switching to Google Fallback (gTTS)...")
            try:
