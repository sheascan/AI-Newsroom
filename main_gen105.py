...
# --- VERSION TRACKER ---
VERSION_ID = "Gen 105 (The Semantic Core)"
...
# Note: Checkpoint file usage is modified in this version
...
# MODEL CONFIGURATION
MODEL_NAME = "gemini-2.0-flash-lite" # Fast model for generation
EMBEDDING_MODEL = "text-embedding-004" # Standard embedding model

SLEEP_TIME = 5 # Reduced sleep time as we use less LLM calls
...
class ContentClusterer:
    ...
    def merge_orphans(self, embeddings, labels):
        ...
        for _ in range(10):
            ...
            if best_target is not None:
                ...
                # If it fits nowhere, stop trying to merge it (it stays an orphan)
                break
    ...
    def run_clustering(self, articles, embeddings):
        ...
        # Aim for ~12 articles per cluster initially
        ...
        for label in unique_labels:
            ...
            # Label generation placeholder (we will use LLM later to name them)
...
# --- HELPER: EMBEDDING GENERATOR ---
def get_embeddings_batch(texts):
    """Generates embeddings for a list of texts using Google GenAI."""
    try:
        # Batching to avoid payload limits (max 100 usually safe)
        batch_size = 50
        all_embeddings = []
        
        for i in range(0, len(texts), batch_size):
            batch = texts[i:i+batch_size]
            # API expects 'content' and 'model'
            # Note: The syntax for batch embedding varies by SDK version.
            # We iterate simply here for safety if batch endpoint is tricky.
            for text in batch:
...
def ai_cluster_and_summarize(article_objects):
    ...
    # This replaces the LLM Sorter with the ContentClusterer class
    ...
    for tag, articles in clustered_data.items():
        ...
        # We sample the top 3 headlines to give the LLM context
