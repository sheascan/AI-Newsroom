...
try:
    ...
    print("❌ Error: Missing BeautifulSoup.")
...
# --- VERSION TRACKER ---
VERSION_ID = "Gen 212 (Auto-Retry + Offline Fallback)"

# --- OPTIONAL IMPORTS ---
try:
    from mutagen.mp3 import MP3
    from mutagen.id3 import ID3, APIC, TIT2, TPE1, TALB, COMM, error
    HAS_MUTAGEN = True
except ImportError:
    HAS_MUTAGEN = False
...
class ContentClusterer:
    ...
    def run_clustering(self, articles, embeddings):
        ...
        if n_articles < 2:
            print(f"   ⚠️ Too few articles for clustering ({n_articles}). Treating as single topic.")
            final_clusters_map = {"Group_0": articles}
        else:
            ...
            for label in unique_labels:
                ...
                tag = f"Group_{label}"
                raw_clusters[tag] = cluster_articles
...
# --- NEW: KEYWORD CLUSTERING FALLBACK (THE SAFETY NET) ---
def keyword_cluster_fallback(articles):
    print("\n🚨 --- GOOGLE EMBEDDING API FAILED: ACTIVATING FALLBACK MODE ---")
    print("      (Grouping articles by keyword instead of AI Vectors)")
    
    clusters = {}
    
    # 1. Bucket articles by Keyword Category
    for a in articles:
        cat = detect_topic_category(a['headline'])
        if cat not in clusters: clusters[cat] = []
        clusters[cat].append(a)
    
    final_output = {"clusters": []}
    
    # 2. Build the Manifest
    for cat, arts in clusters.items():
        slug_cat = cat if cat != "Default" else "General"
        safe_slug = clean_filename(f"News_Roundup_{slug_cat}")
        
        display_title = f"{cat} News Roundup"
        if cat == "Default": display_title = "General News Roundup"
        
        # Limit to 20 articles per fallback group to keep it manageable
        selected_arts = arts[:20]
        
        print(f"   📌 Group: {display_title:<25} | Items: {len(selected_arts)}")
        
        dossier = ""
        sources = []
        for a in selected_arts:
            dossier += f"HEADLINE: {a['headline']}\nFACTS: {a['full_text'][:600]}\n\n"
            sources.append({"title": a['headline'], "url": a['url']})
...
# --- HELPER: EMBEDDINGS (BATCHED WITH RETRY) ---
def get_embeddings_batch(texts):
    all_embeddings = []
    chunk_size = 50 # Reduced from 100 to help with Rate Limits
        ...
        for attempt in range(3):
            try:
                ...
                if "429" in str(e) or "resource_exhausted" in str(e).lower():
                    print(f"      ⏳ Rate Limit Hit on Chunk {i//chunk_size + 1}. Sleeping 45s...")
                    time.sleep(45)
                    continue # Retry this chunk
                else:
                    print(f"      ❌ Embedding Error: {e}")
                    return None # Hard fail triggers Fallback Engine
...
def attach_cover_art(mp3_path, topic_title, sources=None):
    ...
    try:
        ...
        # --- NEW: ADD SOURCE URLS TO COMMENTS TAG ---
        if sources:
            desc_text = "SOURCES USED IN THIS BRIEFING:\n\n"
            for s in sources:
                desc_text += f"• {s['title']}\n  {s['url']}\n\n"
            
            # COMM tag (Language 'eng', Description 'Sources')
            audio.tags.add(COMM(encoding=3, lang='eng', desc='Sources', text=desc_text))
        # ---------------------------------------------
        
        audio.save()
    except Exception as e:
        print(f"      ⚠️ Failed to attach tags: {e}")
...
def log_debug_html(url, content):
    """Saves HTML content to a debug folder for inspection."""
    try:
...
def ai_cluster_and_summarize(article_objects):
    ...
    # --- TRIGGER FALLBACK IF SMART FAILS ---
    if embeddings is None or len(embeddings) == 0: 
        return keyword_cluster_fallback(valid_articles)
    ...
    try:
        ...
        print(f"   ⚠️ Clustering Math Error: {e}")
        return keyword_cluster_fallback(valid_articles)

# --- STAGE 1 & 2: SCRAPING (UPDATED GEN 205) ---
    ...
    for fp in files:
        ...
        if fp.lower().endswith(".eml"):
            try:
                ...
                print(f"   ⚠️ Failed to parse EML {os.path.basename(fp)}: {e}")
        else:
            try:
...
def fetch_article_text(url):
    ...
    # --- GEN 204: ROBUST REQUEST + NULL BYTE FIX + NAIVE FALLBACK ---
    try:
        ...
        # Correct encoding if requests guessed wrong
        if response.encoding is None or response.encoding == 'ISO-8859-1':
            response.encoding = response.apparent_encoding

        html_content = response.text
        
        # --- FIX: SANITIZE HTML (NULL BYTES) ---
        if html_content:
            html_content = re.sub(r'[\x00-\x08\x0b\x0c\x0e-\x1f]', '', html_content)
        # ----------------------------------------

        if not html_content or len(html_content) < 100:
             log_failure(url, "Empty Text (JS Block or poor load)")
             STATS["scraped_failed"] += 1
             return None
        ...
        try:
            ...
            pass # We will handle failure below
        ...
        if article.text and len(article.text) > 200:
            ...
            # 3. NAIVE FALLBACK (If strict parsing failed)
            try:
                ...
                # Join simple text
                raw_fallback = "\n".join(paragraphs)
                if len(raw_fallback) > 200:
                     parsed_text = raw_fallback
                     print(f"      ⚠️  Used Naive Fallback for: {url[:40]}...")
            except: pass

        if not parsed_text or len(parsed_text) < 100:
             # --- SAVE DEBUG HTML ---
             debug_file = log_debug_html(url, html_content)
             log_failure(url, f"Parsing Failed (Content Extraction) - Saved to {debug_file}")
             STATS["scraped_failed"] += 1
             return None

        # Continue with whatever text we found
        is_junk = any(t in article.title.lower() for t in IGNORE_HEADLINE_TERMS)
        raw_text = clean_text(parsed_text)
        truncated_text = raw_text[:1000]
        if len(raw_text) > 1000: truncated_text += "..."

        # Use fallback title if Newspaper missed it
        final_title = article.title
        if not final_title:
             try:
                soup = BeautifulSoup(html_content, 'html.parser')
                final_title = soup.title.string.strip()
             except: final_title = "Untitled Article"
...
async def main():
    ...
    if data:
        ...
        for c in data["clusters"]:
            # --- GEN 139: ADD TIME STAMP TO FILENAME ---
            fname = f"{TODAY_STR}_{TIME_STR}_{c['file_slug']}.mp3"
            ...
            print("      ⏳ Rate Limit Safety: Pausing 1s...") 
            time.sleep(1) 
            ...
            if script:
                # UPDATED: Pass c["sources"] to audio producer
                await produce_audio(c["topic"], script, fname, selected_cast, sources=c["sources"])
                print(f"      ✅ Saved: {fname}")
                index_items.append({
                    "topic": c['topic'],
                    "filename": fname,
        ...
        if index_items:
            # --- GEN 201: SAVE MANIFEST FOR RSS GENERATOR ---
            # This block allows the RSS generator to read the data without parsing HTML
            manifest_path = os.path.join(TODAY_OUTPUT_DIR, "manifest.json")
            manifest_data = {
                "date": TODAY_STR,
                "episodes": []
            }
            ...
            for item in index_items:
                filename_only = os.path.basename(item['filename'])
