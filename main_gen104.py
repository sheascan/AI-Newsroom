...
# --- VERSION TRACKER ---
VERSION_ID = "Gen 104 (Circuit Breaker Edition)"
...
# If EdgeTTS fails once, this turns False for the rest of the script run.
EDGE_TTS_ALIVE = True 
...
# --- STAGE 5: THE SOUND ENGINEER (UPDATED: CIRCUIT BREAKER) ---
async def produce_audio(topic, script, filename):
    # This allows us to modify the global switch if Edge dies
    ...
    for i, line in enumerate(script):
        ...
        # --- PHASE 1: Try EdgeTTS (Only if ALIVE) ---
        if EDGE_TTS_ALIVE:
            try:
                ...
                print(f"      ⚠️ EdgeTTS Failed on this line: {e}")
                print("      🚫 [CIRCUIT BREAKER] Permanently disabling EdgeTTS for this session.")
                EDGE_TTS_ALIVE = False
                # No return/break here; we flow down to Phase 2 for this specific line
        
        # --- PHASE 2: Fallback (gTTS / Robotic) ---
        # Runs if EdgeTTS just died, OR if it was already dead
        if segment is None and HAS_GTTS:
            if not EDGE_TTS_ALIVE:
                # If we are here because of the circuit breaker, imply we are using fallback
                pass 
