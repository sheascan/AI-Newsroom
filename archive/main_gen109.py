...
# --- VERSION TRACKER ---
VERSION_ID = "Gen 109 (Stealth Lawnmower)"
...
CHECKPOINT_FILE = os.path.join(CACHE_DIR, "gen109_clusters.json")
...
def fetch_article_text(url):
    ...
    try:
        ...
        # Check for "Empty" scrapes (common with blocking)
        ...
        # print(f"DEBUG: Scrape fail for {url} -> {e}") 
        STATS["scraped_failed"] += 1
        return None
...
def ai_cluster_and_summarize(article_objects):
    ...
    for tag, arts in raw_clusters.items():
        ...
        # Build Dossier (Lawnmower version)
        full_dossier = ""
        cluster_sources = [] 
