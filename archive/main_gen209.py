...
# --- VERSION TRACKER ---
VERSION_ID = "Gen 209 (Active Model Health Check)"
...
# --- NEW: ACTIVE MODEL HEALTH CHECK ---
def resolve_embedding_model():
    """Tests potential models to find one that actually works."""
    CANDIDATES = [
        "models/text-embedding-004",  # Preferred
        "models/embedding-001",       # Backup
    ]
    
    print("      🕵️  Testing embedding models...")
    for model in CANDIDATES:
        try:
            # Try to embed a single word to check health
    ...
    print("      ❌ ALL EMBEDDING MODELS FAILED. Cannot proceed.")
    return None
...
def ai_cluster_and_summarize(article_objects):
    ...
    active_embedding_model = resolve_embedding_model()
    if not active_embedding_model:
        print("   ❌ Critical Error: No working embedding model found.")
        return None
