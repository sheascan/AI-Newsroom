...
# --- VERSION TRACKER ---
VERSION_ID = "Gen 29 (The Enforcer: Retroactive Filtering)"
...
IGNORE_HEADLINE_TERMS = [
    ...
    # Newsletters / Meta
    "newsletter", "metro sport", "metro uk (@metro.co.uk)", "instagram",
    ...
    # Specific Puzzles / Games / Recurring junk
    "crossword", "spelling bee", "connections — the", "strands: uncover words",
    "minichess", "sudoku", "wordle",
    
    # Specific Publishers (if you want to exclude their meta-pages)
...
def fetch_article_text(url):
    ...
    try:
        ...
        for term in IGNORE_HEADLINE_TERMS:
            # FIX: Ensure term is also lowercase for case-insensitive match
...
def ai_cluster_and_summarize(article_objects):
    ...
    for obj in article_objects:
        ...
        # Even if it was "scraped" (kept) before, we check against the NEW list.
        if status != 'junk':
            for term in IGNORE_HEADLINE_TERMS:
        ...
        # If it's definitely junk now, skip duplicate check
        if status == 'junk':
            obj['status'] = status
