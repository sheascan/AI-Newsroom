...
import requests
import csv
import difflib  # <--- NEW: For comparing headlines
...
# --- VERSION TRACKER ---
VERSION_ID = "Gen 27 (The Refinery: Deduping & Junk Filtering)"
...
# --- HEADLINE FILTERS (Level 2 Filtering) ---
# If the *Title* contains these, we drop it. Good for removing marketing/junk.
IGNORE_HEADLINE_TERMS = [
    # "sale", "off", 
    "discount", "webinar", "register now", "last chance", 
    "giveaway", "promo", "deal of the day", "subscribe",
     # "newsletter",
...
# MODEL CONFIGURATION
# Using the standard 2.0-flash (Assuming it works, otherwise swap to exp-1206)
MODEL_NAME = "gemini-exp-1206"

# STRATEGY: High Volume, Low Frequency
BATCH_SIZE = 40     
SLEEP_TIME = 30     
...
def load_all_inputs(target_path):
    ...
    filtered_raw = []
    for u in master_raw_urls:
        u_lower = u.lower()
        if not any(t in u_lower for t in IGNORE_URL_TERMS):
            filtered_raw.append(u)
...
def fetch_article_text(url):
    ...
    try:
        ...
        if any(term in headline.lower() for term in IGNORE_HEADLINE_TERMS):
            print(f"      🗑️  Junk Headline Removed: {headline[:80]}...")
            return None # Return None means "Skip this article"
...
def ai_cluster_and_summarize(article_objects):
    ...
    for obj in article_objects:
        ...
        is_dupe = False
        current_headline = obj['headline'].lower()
        
        # Check against existing headlines using Fuzzy Matching
        # "SpaceX Launches Rocket" vs "Rocket Launch by SpaceX"
        for seen in seen_headlines:
            ratio = difflib.SequenceMatcher(None, current_headline, seen).ratio()
            if ratio > 0.8: # 80% similarity threshold
                is_dupe = True
                print(f"      🗑️  Duplicate Story Removed: {obj['headline'][:80]}...")
                break
        
        if not is_dupe:
            seen_headlines.append(current_headline)
