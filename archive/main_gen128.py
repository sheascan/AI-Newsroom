...
# --- VERSION TRACKER ---
VERSION_ID = "Gen 128 (Audio Engine Debug)"
...
# VOICE CAST
# UPDATED: Swapped Bob to "GuyNeural" (US Male) to test if "Ryan" (UK Male) was the failure point.
VOICE_CAST = {
...
async def produce_audio(topic, script, filename, cast_dict):
    ...
    # Dynamic Tracking for hallucinations
    unknown_map = {}
    unknown_count = 0
    seen_speakers = set()
    ...
    for line in script:
        ...
        # 4. FINAL FALLBACK: Dynamic Assignment
        if not voice:
        ...
        # --- LOUD AUDIO LOGGING ---
        # We print this for EVERY line to confirm exactly what edge_tts is doing
        # print(f"      🔊 Generating: {spk_clean[:15]:<15} using {voice}")
