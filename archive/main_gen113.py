...
# --- VERSION TRACKER ---
VERSION_ID = "Gen 113 (Forensics Edition)"
...
CHECKPOINT_FILE = os.path.join(CACHE_DIR, "gen113_clusters.json")
...
class ContentClusterer:
    # UPDATED: Split limit lowered to 15
    ...
    def split_giant_clusters(self, articles, embeddings, labels):
        ...
        next_label_id = max(unique_labels) + 1 if len(unique_labels) > 0 else 1
        
        for label in unique_labels:
            indices = np.where(final_labels == label)[0]
            if len(indices) > self.max_size:
                sub_embeddings = embeddings[indices]
                # Force split into 2 sub-topics
