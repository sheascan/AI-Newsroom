...
# --- STAGE 3: THE EDITOR (GEN 10: VOLUME-BASED MERGING) ---
def ai_cluster_and_summarize(article_objects):
    ...
    safety_settings = [types.SafetySetting(category="HARM_CATEGORY_HATE_SPEECH", threshold="BLOCK_NONE")] # (Simplified for brevity, code uses all 4)
    # Re-adding full safety block for robustness
    safety_settings = [
    ...
    sort_input = ""
    for i, obj in indexed_articles.items(): sort_input += f"ID [{i}]: {obj['headline']}\n"
    ...
    try:
        response = client.models.generate_content(
            model="gemini-2.5-flash", contents=sort_prompt,
        ...
        print(f"   ⚠️ Phase A Failed: {e}. Chunking...")
        ids = list(indexed_articles.keys())
        for i in range(0, len(ids), 50): sorted_buckets[f"Batch_{i}"] = ids[i:i+50]

    # --- PHASE B: THE PLANNER (GEN 10 UPDATE) ---
    ...
    for theme, ids in sorted_buckets.items():
        ...
        # GEN 10 LOGIC: FORCE SINGLE CLUSTER FOR SMALL BUCKETS
        if len(valid_ids) <= 10:
        ...
        try:
            response = client.models.generate_content(
                model="gemini-2.5-flash", contents=bucket_prompt,
                config=types.GenerateContentConfig(response_mime_type="application/json", safety_settings=safety_settings)
            )
            if response.text:
                clean_text = response.text.replace("```json", "").replace("```", "").strip()
        ...
        except Exception as e: print(f"      ⚠️ Failed to cluster {theme}: {e}")
