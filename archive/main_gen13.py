...
# --- OPTIONAL IMPORTS (Prevents crash if mutagen is missing) ---
try:
...
# --- HELPER: API RATE LIMIT WRAPPER (EXPONENTIAL BACKOFF) ---
    ...
    for attempt in range(retries):
        try:
            ...
            if "429" in error_str or "RESOURCE_EXHAUSTED" in error_str:
                wait_time = base_wait * (2 ** attempt) # 30, 60, 120...
...
def fetch_article_text(url):
    try:
        ...
        # Reduced snippet size to 300 to reduce token load
...
# --- STAGE 3: THE EDITOR (GEN 13: BATCHED SORTER) ---
def ai_cluster_and_summarize(article_objects):
    ...
    # --- PHASE A: THE SORTER (BATCHED) ---
    print("   -> Phase A: Sorting into broad themes (Batch Mode)...")
    
    # We create a master dict to hold the sorted results
    master_buckets = {}
    
    # Process in batches of 40 to prevent Token Limit Errors
