...
# --- VERSION TRACKER ---
VERSION_ID = "Gen 120 (Smart Naming & Flash 2.5)"
...
# --- MODEL SELECTION ---
# Primary: Flash Lite (Fastest/Cheapest). Fallback: Flash 2.5 (Standard)
...
# --- HELPER: CLEAN FILENAME (NEW IN GEN 120) ---
def clean_filename(text, max_length=50):
    """
    Converts a topic string into a safe, readable filename.
    Ex: "Blue Origin's next flight!" -> "blue_origins_next_flight"
    """
    if not text: return "untitled_topic"
    
    # Lowercase and remove non-alphanumeric chars (preserve spaces/hyphens)
...
# --- HELPER: API WRAPPER (UPDATED FOR GEN 120) ---
def generate_with_retry(model_name, contents, config, retries=3):
    """
    Retry logic with LOUD feedback so the user knows it's not frozen.
    Updated to fallback to 2.5-flash if lite fails.
    """
    base_wait = 15  
...
def ai_cluster_and_summarize(article_objects):
    ...
    for tag, arts in raw_clusters.items():
        ...
        # USE FIRST HEADLINE AS TOPIC
        topic_name = arts[0]['headline']
        
        # GEN 120: USE CLEAN FILENAME INSTEAD OF "GROUP_X"
        # We calculate the clean slug here for reference, but use it later in main()
        ...
        print(f"{tag:<15} | {len(arts):<5} | {display_name}")
        ...
        final_clusters["clusters"].append({
            "topic": topic_name, 
            "file_slug": safe_topic, # Updated to use the clean topic
...
async def main():
    ...
    for c in data["clusters"]:
        # GEN 120: NEW FILE NAMING CONVENTION
        # Uses the clean slug from the clustering stage
        fname = f"{TODAY_STR}_{c['file_slug']}.mp3"
        
        print(f"\n📺 Studio: Producing '{c['topic'][:50]}...'")
        
        # --- FORCED RATE LIMIT PAUSE ---
        # Even with retries, we enforce a mandatory polite pause to keep RPM low.
