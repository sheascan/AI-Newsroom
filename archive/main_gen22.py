...
# --- VERSION TRACKER ---
VERSION_ID = "Gen 22 (Checkpoint Edition)"
...
CHECKPOINT_FILE = os.path.join(CACHE_DIR, "phase_a_checkpoint.json") # <--- NEW SAVE POINT
...
BATCH_SIZE = 10     
SLEEP_TIME = 20     # 20s Pause for safety
...
# --- STAGE 3: THE EDITOR (GEN 22: CHECKPOINT EDITION) ---
def ai_cluster_and_summarize(article_objects):
    ...
    # 2. Run Phase A if no checkpoint
    if not master_buckets:
        ...
        # Save Checkpoint after successful sort
        if master_buckets:
    ...
    # Cleanup checkpoint only if we successfully made clusters
