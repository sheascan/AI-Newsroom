...
# --- VERSION TRACKER ---
VERSION_ID = "Gen 130 (Misc Roundup Logic)"
...
class ContentClusterer:
    ...
    def merge_orphans(self, embeddings, labels):
        ...
        # We rely on this to group "close" items. 
        # Anything that fails this check will be caught by the "Misc" collector later.
        for _ in range(10):
...
# --- STAGE 3: CLUSTERER (WITH MISC CLEANUP) ---
def ai_cluster_and_summarize(article_objects):
    ...
    # --- NEW: POST-CLUSTERING CLEANUP ---
    # We collect any cluster with < 3 items into a single "Misc" group
    final_clusters_map = {}
    misc_articles = []
...
# --- STAGE 1 & 2: SCRAPING ---
# (Standard scraping functions retained)
...
async def produce_audio(topic, script, filename, cast_dict):
    ...
    for line in script:
        ...
        # --- DEBUG LOGGING (Once per speaker only) ---
        if raw_spk not in seen_speakers:
            seen_speakers.add(raw_spk)
            # Find which name this voice belongs to for the log
