...
# --- VERSION TRACKER ---
VERSION_ID = "Gen 111 (Black Box Recorder)"
...
CHECKPOINT_FILE = os.path.join(CACHE_DIR, "gen111_clusters.json")
...
STATS = {
    ...
    "raw_urls": 0,
    "unique_urls": 0,
    "scraped_success": 0, # Freshly scraped
    "scraped_cached": 0,  # Loaded from cache
    "scraped_failed": 0,
    "junk_removed": 0,
    "dupes_removed": 0,
    "clusters_created": 0
...
# --- STAGE 2: SCRAPER (BLACK BOX EDITION) ---
def log_failure(url, reason):
    """Writes failed URLs to a CSV so we know WHY they failed."""
    file_exists = os.path.exists(FAILURE_LOG_FILE)
    try:
        with open(FAILURE_LOG_FILE, 'a', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            if not file_exists: writer.writerow(["Timestamp", "URL", "Reason"])
            writer.writerow([datetime.datetime.now().strftime("%H:%M:%S"), url, reason])
    except: pass
...
def fetch_article_text(url):
    ...
    # 2. RANDOM DELAY (Stealth)
    time.sleep(random.uniform(0.5, 2.0))

    try:
        # 3. PRE-FLIGHT CHECK (Is the URL dead?)
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'}
        try:
            # Short timeout to quickly weed out dead links
            r = requests.head(url, headers=headers, timeout=5, allow_redirects=True)
            if r.status_code >= 400:
                log_failure(url, f"Status Code {r.status_code}")
                STATS["scraped_failed"] += 1
                return None
        except: pass # Ignore request errors, let newspaper try

        # 4. SCRAPE
        conf = Config()
        conf.browser_user_agent = headers['User-Agent']
        ...
        if not article.text or len(article.text) < 100:
             # Often caused by Javascript-only sites
             log_failure(url, "Empty/Too Short Text (JS issue?)")
             STATS["scraped_failed"] += 1
             return None
        ...
        log_failure(url, f"Exception: {str(e)[:50]}")
        STATS["scraped_failed"] += 1
        return None
...
def ai_cluster_and_summarize(article_objects):
    ...
    if not valid_articles: 
        print("❌ No valid articles after filtering.")
        return None
...
def generate_script(topic, dossier, cast_names, source_count):
    ...
    # Strict Limit: 30k chars is enough for context but safe for API
...
async def main():
    ...
    if STATS['scraped_failed'] > 0:
        print(f"      ⚠️  See {FAILURE_LOG_FILE} for details.")
