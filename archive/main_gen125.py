...
# --- VERSION TRACKER ---
VERSION_ID = "Gen 125 (Fuzzy Voice Matching)"
...
# --- FILTERING ---
IGNORE_URL_TERMS = ["unsubscribe", "privacy policy", "terms", "opt_out", "login", "signup", C "facebook.com", "twitter.com", "instagram.com", "linkedin.com", "youtube.com", "google.com", "apple.com", "tiktok.com", "arxiv.org"]
...
class ContentClusterer:
    ...
    def merge_orphans(self, embeddings, labels):
        ...
        unique_labels, counts = np.unique(refined_labels, return_counts=True)
        sizes = dict(zip(unique_labels, counts))
        orphans = [l for l, s in sizes.items() if s < 3] 
        
        if orphans and len(sizes) > 1:
            largest_label = max(sizes, key=sizes.get)
            for o in orphans:
                if o != largest_label:
                     refined_labels[refined_labels == o] = largest_label
...
async def produce_audio(topic, script, filename, cast_dict):
    ...
    # Create a normalized map for easier lookups
    # e.g. {"alice": voice_obj, "bob": voice_obj}
    clean_map = {}
    ...
    # Map generic fallbacks just in case
    ...
    for line in script:
        ...
        # 1. Clean the speaker name
        spk_clean = raw_spk.strip().lower().replace(":", "")
        
        # 2. Try map lookup
        voice = clean_map.get(spk_clean)
        
        # 3. If missing, try smarter substring matching
        if not voice:
            # Check if Male Host Name is inside the string (e.g. "Host (Chris)")
            if host_keys[1].lower() in spk_clean:
                voice = cast_dict[host_keys[1]]
            # Check if Female Host Name is inside
            elif host_keys[0].lower() in spk_clean:
                voice = cast_dict[host_keys[0]]
            # Total failure -> Default to Female
            else:
                voice = cast_dict[host_keys[0]]
...
async def main():
    ...
    for c in data["clusters"]:
        ...
        if topic_category == "Default":
             # ROLL THE DICE: Pick any available cast pair from the dictionary
