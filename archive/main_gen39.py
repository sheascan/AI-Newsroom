...
# --- VERSION TRACKER ---
VERSION_ID = "Gen 39 (Robust Sorting & JSON Fix)"
...
def generate_index_html(metadata):
    if not os.path.exists(TODAY_OUTPUT_DIR): os.makedirs(TODAY_OUTPUT_DIR)
    idx_path = os.path.join(TODAY_OUTPUT_DIR, "index.html")
    html = f"<html><head><title>Podcast {TODAY_STR}</title></head><body><h1>Podcast Edition: {TODAY_STR}</h1>"
    for m in metadata:
        html += f"<div><h2>{m['topic']}</h2><audio controls src='{m['filename']}'></audio><ul>"
        for u in m['source_urls']:
            html += f"<li><a href='{u}'>{u[:50]}...</a></li>"
        html += "</ul></div>"
    html += "</body></html>"
    with open(idx_path, "w", encoding='utf-8') as f: f.write(html)

def archive_inputs(file_paths):
    if not os.path.exists(TODAY_INPUT_ARCHIVE): os.makedirs(TODAY_INPUT_ARCHIVE)
    for fp in file_paths:
        try:
            shutil.move(fp, os.path.join(TODAY_INPUT_ARCHIVE, os.path.basename(fp)))
        except: pass
    print(f"   📦 Archived processed inputs.")
...
def load_all_inputs(target_path):
    ...
    print(f"   -> Found {len(raw_urls)} raw links. De-duplicating...")
    ...
    print(f"   -> Consolidated into {len(final_urls)} clean, unique articles.")
...
def ai_cluster_and_summarize(article_objects):
    ...
    if not master_buckets:
        ...
        for i in range(0, len(ids), BATCH_SIZE):
            ...
            # IMPROVED PROMPT
            prompt = "Sort these articles into broad categories: [World, Politics, Tech, Sport, Culture, Economy, Science]. \nReturn a JSON object where keys are the categories and values are LISTS OF INTEGERS (the IDs). \nExample JSON: {\"World\": [1, 2, 5], \"Politics\": [3, 4]}\n\nARTICLES:\n"
            ...
            if resp:
                try:
                    ...
                    if json_start != -1 and json_end != -1:
                        clean_text = text_resp[json_start:json_end]
                    else:
                        clean_text = clean_json_response(text_resp) # Fallback

                    parsed_batch = json.loads(clean_text)
                    
                    for k, v in parsed_batch.items():
                        master_buckets.setdefault(k, []).extend(v)
                        
                except json.JSONDecodeError:
                    print(f"      ❌ JSON Parse Error. Raw Output snippet: {resp.text[:100]}...")
                except Exception as e:
    ...
    for theme, ids in master_buckets.items():
        ...
        for x in flat_list:
            found_digits = re.findall(r'\d+', str(x))
            if found_digits:
                extracted_id = int(found_digits[0])
                if extracted_id in indexed_articles:
                    valid_ids.append(str(extracted_id))
    ...
    for theme, ids in merged_buckets.items():
        ...
        prompt = f"Cluster these '{theme}' articles into coherent podcast segments/episodes. \nReturn JSON: {{ 'clusters': [ {{ 'topic': 'Brief Topic Title', 'file_slug': 'short_filename_slug', 'ids': [1, 2] }} ] }}\n\nARTICLES:\n"
        ...
        if resp:
            try:
                ...
                json_end = text_resp.rfind('}') + 1
                if json_start != -1 and json_end != -1:
                    clean = text_resp[json_start:json_end]
                else:
                    clean = clean_json_response(text_resp)
                ...
                for c in json.loads(clean).get("clusters", []):
                    ...
                    raw_ids = c.get("ids", [])
                    for rid in raw_ids:
                        found = re.findall(r'\d+', str(rid))
                        if found: cluster_ids.append(int(found[0]))

                    for aid in cluster_ids:
                        if aid in indexed_articles:
                            art = indexed_articles[aid]
                    ...
                    if dossier:
                        final_clusters["clusters"].append({
                            ...
                            "file_slug": c.get("file_slug", "news_update"), 
                            "dossier": dossier, 
