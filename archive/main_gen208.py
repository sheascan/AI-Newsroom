...
# --- VERSION TRACKER ---
VERSION_ID = "Gen 208 (Auto-Discovery of Embedding Models)"
...
# NOTE: EMBEDDING_MODEL is now determined dynamically in get_dynamic_embedding_model()
...
class ContentClusterer:
    ...
    def run_clustering(self, articles, embeddings):
        ...
        # --- FIX: HANDLE SINGLE ARTICLE EDGE CASE ---
        if n_articles < 2:
...
# --- NEW: DYNAMIC MODEL DISCOVERY (Fixes 404 Errors) ---
def get_dynamic_embedding_model():
    """Asks Google which embedding models are actually available."""
    print("      🕵️  Resolving available embedding models...")
    try:
        current_models = list(client.models.list())
        candidates = []
        ...
        if not candidates:
            print("      ⚠️ No specific embedding models found. Trying default...")
            return "models/text-embedding-004"
            
        # Prioritize newer models
        for c in candidates:
            if "text-embedding-004" in c: 
                print(f"      ✅ Selected: {c}")
                return c
        for c in candidates:
            if "text-embedding" in c: 
                print(f"      ✅ Selected: {c}")
                return c
                
        # Last resort: grab the first one
        print(f"      ✅ Selected (Fallback): {candidates[0]}")
        return candidates[0]
        
    except Exception as e:
        print(f"      ⚠️ Model discovery failed ({e}). using hardcoded default.")
        return "models/text-embedding-004"

# --- HELPER: EMBEDDINGS (BATCHED & DYNAMIC) ---
def get_embeddings_batch(texts, model_name):
    ...
    try:
        ...
        print(f"      ❌ Embedding Failed ({model_name}): {e}")
        return None
...
def ai_cluster_and_summarize(article_objects):
    ...
    active_embedding_model = get_dynamic_embedding_model()

    print(f"   -> Embedding {len(valid_articles)} valid articles...")
    headlines = [f"{a['headline']}: {a['snippet']}" for a in valid_articles]
    
    embeddings = get_embeddings_batch(headlines, active_embedding_model)
...
# --- STAGE 5: AUDIO ---
async def produce_audio(topic, script, filename, cast_dict, sources=None):
    ...
    # UPDATED: Pass sources to the tagging function
    attach_cover_art(path, topic, sources)
    return filename
