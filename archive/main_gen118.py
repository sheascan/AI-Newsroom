...
# --- VERSION TRACKER ---
VERSION_ID = "Gen 118 (Quota Survival Mode)"
...
CHECKPOINT_FILE = os.path.join(CACHE_DIR, "gen118_clusters.json")
...
# --- MODEL SELECTION ---
# Switching to "8b" (Nano) model for higher throughput and lower quota usage
    ...
    for attempt in range(retries):
        try:
            ...
            if "429" in e_str or "resource_exhausted" in e_str:
                ...
                print(f"      ❌ Model Error: {model_name} not found. Trying fallback...")
                # Fallback to standard 1.5 Flash if 8b is missing
                if model_name != "gemini-1.5-flash":
                    return generate_with_retry("gemini-1.5-flash", contents, config, retries-1)
                else:
                    return None
            else:
...
def ai_cluster_and_summarize(article_objects):
    ...
    for tag, arts in raw_clusters.items():
        ...
        # --- OPTIMIZATION: SKIP NAMING API CALL ---
        # Instead of asking AI to name it (Quota Heavy), we just use the first headline.
        topic_name = arts[0]['headline']
        if len(topic_name) > 50: topic_name = topic_name[:47] + "..."
        file_slug = f"Cluster_{tag}"
        # -------------------------------------------
...
async def main():
    ...
    for c in data["clusters"]:
        ...
        # --- FORCED COOLDOWN BETWEEN GENERATIONS ---
        # This prevents blasting the API with back-to-back 1200-word generation requests
        print("      ⏳ Cooling down API (15s)...")
        time.sleep(15)
        # -------------------------------------------
