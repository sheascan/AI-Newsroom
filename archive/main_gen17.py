...
# MODEL CONFIGURATION
# We revert to 2.5-flash because your logs proved it exists (returned 429, not 404).
MODEL_NAME = "gemini-2.5-flash"
BATCH_SIZE = 5      # Ultra-small batches to prevent token spikes
SLEEP_TIME = 25     # 25s pause guarantees we stay under the 20 RPM limit
...
# --- STAGE 3: THE EDITOR (GEN 17: ULTRA-SAFE MODE) ---
def ai_cluster_and_summarize(article_objects):
    ...
    # --- PHASE A: THE SORTER (MICRO BATCHES) ---
    print("   -> Phase A: Sorting into broad themes (Ultra-Safe Mode)...")
    
    master_buckets = {}
    
    ids = list(indexed_articles.keys())
    
    # BATCH SIZE: 5 (Very Small)
    for i in range(0, len(ids), BATCH_SIZE):
        ...
        # MANDATORY LONG PAUSE to reset RPM count
