...
# --- VERSION TRACKER ---
VERSION_ID = "Gen 36 (Hotfix: JSON Parser Restored)"
...
IGNORE_HEADLINE_TERMS = [
    ...
    "metro sport", "metro uk (@metro.co.uk)", "instagram", "make your day", 
    "metro uk newsletters", "terms and conditions", "cookies policy", 
    "terms of use", "preferences", "ai & tech newsletter", "courses on chatgpt", 
    "techpssdec2", "workos", "work with us", "privacy policy", "sign up for", 
    "email newsletters", "ad choices", "crossword", "spelling bee", 
    "connections — the", "strands: uncover words", "minichess", "sudoku", "wordle",
    "the guardian view on", "the long read", "today in focus", "new york times", 
    "the new york", "the irish times", "one ai to", "walk mate", "visualize knowledge", 
    "your kitchen assistant", "enable ai", "proofly", "free guide to",
...
# --- HELPER: CLEAN JSON (THE FIX) ---
def clean_json_response(text):
    """Strips Markdown formatting from API response."""
...
def check_and_clean_storage():
    ...
    if total_mb > STORAGE_LIMIT_MB:
        print("⚠️  Limit Exceeded! Cleaning needed.")
...
def ai_cluster_and_summarize(article_objects):
    ...
    # Phase A: Sorting with MEGA BATCHES
    master_buckets = {}
    ...
    if not master_buckets:
        ...
        for i in range(0, len(ids), BATCH_SIZE):
            ...
            if resp:
                try:
                    clean = clean_json_response(resp.text) # <--- JSON FIX APPLIED
    ...
    for theme, ids in master_buckets.items():
        valid_ids = [x for x in ids if isinstance(x, int) or x.isdigit()]
...
async def produce_audio(topic, script, filename):
    ...
    for i, line in enumerate(script):
        ...
        # 2. Fallback (If Edge failed or is banned)
...
async def main():
    ...
    target = INPUT_DIR
    # Fix for path argument passing
    if len(sys.argv) > 1 and sys.argv[1] == "RESUME":
        print("🔄 Resuming from cache...")
        data = load_state()
        files = [] # Skip archiving on resume
    else:
