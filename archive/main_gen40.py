...
import csv
import difflib
import math # Added for cluster math
...
# --- VERSION TRACKER ---
VERSION_ID = "Gen 40 (High-Yield Harvest & Smart Batching)"
...
# --- FILTERING CONFIG (REFINED) ---
IGNORE_URL_TERMS = [
    "unsubscribe", "manage_preferences", "opt_out", "login", "signup", "signin",
    "facebook.com", "twitter.com", "instagram.com", "linkedin.com", "youtube.com",
    "tiktok.com", "help center", "contact us", "advertisement", "click here",
    "/author/", "/tag/", "/category/", "/section/" # Ignore index pages, get the stories
]

# REMOVED Brand names (NYT, Guardian) so we don't kill stories ABOUT them.
IGNORE_HEADLINE_TERMS = [
    "sale", "off", "discount", "webinar", "register now", "last chance", 
    "giveaway", "promo", "deal of the day", "subscribe", "newsletter",
    "terms and conditions", "cookies policy", "privacy policy",
    "wordle", "crossword", "spelling bee", "connections — the", "sudoku",
    "horoscope", "today's top stories", "briefing", "evening briefing",
    "manage your account", "view in browser"
]
...
# MODEL CONFIGURATION
# Using Flash Lite Preview for speed and higher limits
MODEL_NAME = "gemini-2.0-flash-lite-preview" 

BATCH_SIZE = 50 # Increased batch size for sorting
SLEEP_TIME = 15     
...
def load_all_inputs(target_path):
    ...
    for url in master_raw_urls:
        try:
            ...
            # REMOVED: strict depth check. 
            # We now only reject if it is purely the homepage
            if clean_url.strip("/") == f"{parsed.scheme}://{parsed.netloc}":
                continue
...
def fetch_article_text(url):
    ...
    try:
        ...
        # --- LEVEL 2: HEADLINE FILTERING ---
        is_junk = False
        reject_reason = ""
        
        # Only junk if short generic headline OR explicit ignore term
        if len(headline) < 15:
            is_junk = True
            reject_reason = "Headline too short"
        else:
...
# --- STAGE 3: THE EDITOR (GEN 40 UPGRADE) ---
def ai_cluster_and_summarize(article_objects):
    
    # --- LEVEL 3: DEDUPLICATION ---
    print("   -> Running Refining Pass (Removing Duplicates)...")
    ...
    for obj in article_objects:
        if not obj: continue
        if obj.get('status') == 'junk': continue

        headline_lower = obj['headline'].lower()

        # --- DEDUPLICATION (Fuzzy but strict) ---
        is_dupe = False
        for seen in seen_headlines:
            ratio = difflib.SequenceMatcher(None, headline_lower, seen).ratio()
            # Increased ratio: Only dedupe if 85% match (avoid merging distinct updates)
            if ratio > 0.85:
                is_dupe = True
    ...
    if os.path.exists(CHECKPOINT_FILE):
        ...
        except: master_buckets = {}

    if not master_buckets:
        print(f"   -> Phase A: Sorting {len(indexed_articles)} items into broad themes...")
        ...
        for i in range(0, len(ids), BATCH_SIZE):
            ...
            sort_prompt = """
            Sort these headlines into ONE of these buckets:
            [World, Politics, Tech, Sport, Culture, Economy, Science].
            If unsure, put in 'General'.
            ...
            if response and response.text:
                try:
                    batch_buckets = json.loads(response.text)
                    for category, id_list in batch_buckets.items():
                        if category not in master_buckets: master_buckets[category] = []
                        if isinstance(id_list, list): master_buckets[category].extend(id_list)
                except Exception as e:
    ...
    # --- PHASE B: THE PLANNER (5-10 Rule) ---
    final_clusters = {"clusters": []}
    
    for theme, ids in master_buckets.items():
        valid_ids = list(set([int(x) for x in ids if int(x) in indexed_articles]))
        count = len(valid_ids)
        if count == 0: continue
        
        # --- THE 5-10 RULE ---
        # We want approx 7 articles per episode.
        target_episodes = math.ceil(count / 8) 
        if count < 5: target_episodes = 1
        
        print(f"   -> Phase B: Clustering '{theme}' ({count} items) into {target_episodes} Episodes...")
        ...
        bucket_prompt = f"""
        You are the Executive Producer for '{theme}'.
        You have {count} articles. 
        Create exactly {target_episodes} distinct podcast episodes from these stories.
        Each episode should group 5-10 related stories together.
        
        OUTPUT JSON: 
        {{ "clusters": [ 
            {{ "topic": "Brief Theme Title", "file_slug": "Slug", "article_ids": [1, 2, 5] }},
            {{ "topic": "Another Theme Title", "file_slug": "Slug", "article_ids": [8, 9] }}
        ] }}
        ...
        if response and response.text:
            try:
                bucket_data = json.loads(response.text)
                clusters = bucket_data.get("clusters", [])
                
                for c in clusters:
                    full_dossier = ""
                    source_urls = []
                    c_ids = c.get("article_ids", [])
                    for art_id in c_ids:
...
def generate_script(topic, dossier, cast_names, source_count):
    ...
    # Logic for script length based on article count
    if source_count <= 4:
        duration_desc = "4-minute 'Flash Update'"
        word_count = "500 words"
    elif source_count <= 8:
        duration_desc = "8-minute 'Standard Briefing'"
        word_count = "1100 words"
    else:
        duration_desc = "15-minute 'Deep Dive'"
        word_count = "1800 words"
    ...
    HOSTS: {host1} & {host2}.
    INSTRUCTIONS: 
    1. Group the stories logically with smooth transitions.
    2. Use the provided DOSSIER facts. Do not make up info.
    ...
    if response and response.text:
        ...
        with open(vault_path, 'w') as f: json.dump(script_data, f, indent=4)
        return script_data
    return []
...
async def produce_audio(topic, script, filename):
    ...
    print("   🎹 Mixing background tracks...")
    ...
    if available_tracks and len(combined_vocals) > 0:
        ...
        while len(music_mix) < target_duration_ms:
            ...
            try:
                ...
                else: music_mix = music_mix.append(track, crossfade=3000)
            except: break
...
async def main():
    ...
    # Clean Phase A checkpoint to ensure fresh sort with new logic
    if os.path.exists(CHECKPOINT_FILE):
        try: os.remove(CHECKPOINT_FILE)
        except: pass

    urls, processed_files = load_all_inputs(INPUT_DIR)
    if not urls: 
    ...
    if not data or not data.get("clusters"):
        print("❌ No news clusters generated.")
        return
    ...
    if index_metadata: 
        ...
        archive_inputs(processed_files)
