...
# --- VERSION TRACKER ---
VERSION_ID = "Gen 115 (Cleaner URLs + Auto-Model)"
...
CHECKPOINT_FILE = os.path.join(CACHE_DIR, "gen115_clusters.json")
...
# --- AUTO-DISCOVER MODEL ---
def get_best_model(client_obj):
    """
    Queries the API to find a valid model name, avoiding 404s.
    """
    print("   🔍 Auto-Negotiating Model ID...")
    try:
        ...
        for p in preferences:
            if p in available_names:
                print(f"      ✅ Found Valid Model: {p}")
                return p
        
        # 5. Fallback: If no exact match, grab the first "gemini" model found
        for name in available_names:
            if "gemini" in name and "vision" not in name:
                print(f"      ⚠️  Fallback Model: {name}")
                return name
                
        return "gemini-1.5-flash"

    except Exception as e:
        print(f"      ⚠️  Discovery Failed ({e}). Defaulting to 'gemini-1.5-flash'")
        return "gemini-1.5-flash"
