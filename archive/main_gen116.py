...
# --- VERSION TRACKER ---
VERSION_ID = "Gen 116 (Quota Fix + Forced 1.5 Flash)"
...
CHECKPOINT_FILE = os.path.join(CACHE_DIR, "gen116_clusters.json")
...
# EXTENSIONS TO SKIP (Images, Audio, Video, Docs)
BAD_EXTENSIONS = (
    ".jpg", ".jpeg", ".png", ".gif", ".bmp", ".svg", ".webp", ".ico",  # Images
    ".mp3", ".wav", ".mp4", ".avi", ".mov", ".mkv", ".flv",            # Media
    ".zip", ".tar", ".gz", ".rar", ".7z",                              # Archives
    ".pdf", ".doc", ".docx", ".xls", ".xlsx", ".ppt", ".pptx",         # Docs
    ".css", ".js", ".xml", ".json", ".txt"                             # Code/Raw
)
...
# FORCED MODEL SELECTION (Stability > Newness)
# We force 1.5 Flash because it has a 1,500 RPD limit (vs 20 RPD for 2.5)
...
class ContentClusterer:
    ...
    def split_giant_clusters(self, articles, embeddings, labels):
        """
        Recursively splits clusters until all are <= max_size.
        """
        ...
        while True:
            ...
            for giant_label in giants:
                ...
                # Update labels: 0 keeps old ID, 1 gets new ID
...
def generate_with_retry(model_name, contents, config, retries=5):
    """
    Robust generation with explicit handling for 429 Quota errors.
    """
    base_wait = 10  # Increased base wait time
    for attempt in range(retries):
        try:
            ...
            if "429" in str(e) or "RESOURCE_EXHAUSTED" in str(e):
                ...
                # Cap max wait at 60 seconds
                if wait > 60: wait = 60 
                print(f"      ⚠️  Quota Hit (429). Cooling down for {wait}s...")
                time.sleep(wait)
            elif "404" in str(e) and "not found" in str(e).lower():
                print(f"      ❌ Critical Model Error: {model_name} not found.")
                return None
            else:
                print(f"      ⚠️ API Error: {e}")
                # Don't break immediately on generic network blips, try once more
...
def analyze_failures():
    """Reads the CSV log and prints a summary to the console."""
...
def load_all_inputs(target_path):
    ...
    for u in raw_urls:
        # 1. Clean "empty parameter" URLs (ending in =)
        if u.endswith("="):
             u = u.split("?")[0]

        # 2. Basic Term Filter
        if any(x in u.lower() for x in IGNORE_URL_TERMS): 
            continue

        # 3. Extension Filter (Exclude images/media/docs)
        try:
            path = urlparse(u).path.lower()
            if path.endswith(BAD_EXTENSIONS):
                continue
        except:
            pass # If URL parsing fails, skip check or skip URL? Let's skip check.
