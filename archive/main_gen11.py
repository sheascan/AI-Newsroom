...
# --- HELPER: API RATE LIMIT WRAPPER (NEW) ---
def generate_with_retry(model_name, contents, config, retries=5):
    """Wraps Gemini calls with a 'Penalty Box' for 429 errors."""
    for attempt in range(retries):
        try:
            ...
            if "429" in error_str or "RESOURCE_EXHAUSTED" in error_str:
                wait_time = 60 # Safe default for Free Tier
...
# --- STAGE 3: THE EDITOR (GEN 11: PATIENT EDITION) ---
def ai_cluster_and_summarize(article_objects):
    ...
    if response and response.text:
        ...
        print(f"   ⚠️ Phase A Failed. Chunking...")
