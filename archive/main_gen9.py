...
MUSIC_STATS_FILE = os.path.join(DATA_DIR, "music_stats.csv") # <--- NEW TRACKER FILE
...
# --- HELPER: MUSIC TRACKER (NEW) ---
def update_music_stats(track_filename):
    """Updates the CSV usage counter for a specific track."""
    stats = {}
    ...
    if os.path.exists(MUSIC_STATS_FILE):
        ...
        except Exception: pass # Ignore read errors, start fresh

    # 2. Update the specific track
    ...
    try:
        ...
        print(f"      ⚠️ Failed to update music stats: {e}")
...
# --- STAGE 5: THE SOUND ENGINEER (SMART DJ + BRANDING + TRACKING) ---
